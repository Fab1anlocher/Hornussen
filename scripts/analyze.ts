...
import type { Match, Observation, PlayingDirectionsConfig } from "../lib/types";
...
function main() {
  ...
  // build-dataset drops never-played 0:0 rounds; report how many that was.
  const archived = readData<Match[]>("matches.json", []);
  const played = new Set(obs.map((o) => o.matchId)).size;
  const result = analyze(obs, cfg.model.k, Math.max(0, archived.length - played));
